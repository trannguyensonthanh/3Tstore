...
import { getAllUsers, getOrder, getUser } from "../../../service/getcategory/getCategory";
import {
  patchInforV1,
  patchUser,
  patchUserV1,
...
patchCus(
  ...
  const handleDelete = async (id, values) => {
    if (values.type !== "admin") {
      const purchaseUser = await getPur(id);
      if(purchaseUser){
        Modal.error({
          title: "Lỗi",
          content: "Không thể xóa vĩnh viễn user đã đặt hàng",
        });
      }
      else {
